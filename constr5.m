% This m-file contruct the strucutre of whole system,used for iteration2.m
...
%K=4;
T=3;
s_sys=randi(5,K,T)+1.5;  % unit time task requests,random distributed in [1,5]
B_sys=randi(10,K).*1e7/2.5; %/2.5 is added for modulate
for i=1:K
    sys_B(i,i)=0;
end
v=randi(5,1,T)+5;
...
R_sys=randi(5,1,K); %unit time resource commitment,random distributed in [1,5]
Coa_str=zeros(K,K);
...
% test for seeking for appropriate D value
% D=zeros(1,T);
% tao=50;
% for i=1:T
%     D(i)=0.8*(sum(R_sys)*tao)/T/(sum(s_sys(:,i))*tao);
% end

D=[0.2712    0.2745    0.2479];
